...
pygame.mixer.init()
mario_sound = pygame.mixer.Sound("/Users/franciscosamagaio/Documents/GitHub/JumpingBall/Jumping Ball/Jumping ball Mario/mario_jump.wav")  # Ensure the file is in the same directory
mario_sound.play()  # Start playing
pygame.mixer.pause()  # Start paused

music_start_time = None  # Track when music starts
ball_touching = False  # Flag to check if ball is touching
...
if not os.path.exists("Jumping ball Mario/frames"):
    os.makedirs("Jumping ball Mario/frames")
...
while running:
   ...
   # Drawing
    ...
    if frame_count % 2 == 0:
        pygame.image.save(screen, f"Jumping ball Mario/frames/frame_{saved_frame_count:04d}.png")
    ...
    if last_collision_time + 5 < frame_count:
        pygame.mixer.pause() 

pygame.quit()

if os.path.exists("Jumping ball Mario/output.mp4"):
    os.remove("Jumping ball Mario/output.mp4")
subprocess.run([
    "ffmpeg", "-framerate", "60", "-i", "Jumping ball Mario/frames/frame_%04d.png",
    "-c:v", "libx264", "-pix_fmt", "yuv420p", "Jumping ball Mario/output.mp4"
])

# Delete all frames in the "frames" folder
if os.path.exists("Jumping ball Mario/frames"):
    shutil.rmtree("Jumping ball Mario/frames")  # Delete entire folder
